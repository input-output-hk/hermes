...
#define HERMES_BUFFER(x) \
    (hermes_hash_api_bstr_t) { (uint8_t *)x, sizeof(x) }
...
bool test_blake2b_512_function()
{
    ...
    if (hermes_hash_api_blake2b(&buf, &outlen, local_ret, local_err))
    {
        ...
        return memcmp(local_ret, constantData, sizeof(constantData)) == 0;
    }
    return false;
}
