...
impl hermes::exports::hermes::cardano::event_on_block::Guest for TestComponent {
    fn on_cardano_block(
        ...
        hermes::hermes::logging::api::log(
            ...
            None,
            None,
            format!("✈️ - on_cardano_block event trigger - subscription ID: {subscription_id:?}, network: {network:?}, slot: {slot:?}, is rollback: {is_rollback}, is immutable: {is_immutable}").as_str(),
            None,
        );
    }
}
...
impl hermes::exports::hermes::init::event::Guest for TestComponent {
    fn init() -> bool {
        ...
        let subscribe_from = hermes::hermes::cardano::api::SyncSlot::Genesis;
