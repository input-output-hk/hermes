...
use crate::{app::HermesAppName, runtime_extensions::{
    app_config::*,
...
pub(super) fn open(readonly: bool, memory: bool, app_name: HermesAppName) -> Result<*mut sqlite3, OpenError> {
    ...
    let (db_path, config) = if memory {
        let inmemory_config = match get_app_inmemory_sqlite_db_cfg(app_name) {
            Some(config) => config,
            None => {
                return Err(OpenError::InvalidInMemoryConfig)
            },
        };
        ...
        let persistent_config = match get_app_persistent_sqlite_db_cfg(app_name) {
            Some(config) => config,
            None => {
                return Err(OpenError::InvalidPersistentConfig)
            },
        };
        ...
        let db_name = match &persistent_config.db_file {
            ...
            None => {
                return Err(OpenError::MissingDatabaseNameForPersistentConfig)
            },
        };
