...
use crate::{
    database::{
        data::{RbacDbData, RbacStakeDbData},
        SQLITE,
    },
    hermes::{
        self,
...
pub(crate) fn prepare_insert_rbac_registration() -> anyhow::Result<Statement> {
    ...
    SQLITE.prepare(RBAC_INSERT_RBAC_REGISTRATION).map_err(|e| {
        log_error(
            FILE_NAME,
            FUNCTION_NAME,
...
fn bind_rbac_registration(
    ...
    bind_with_log(stmt, FUNCTION_NAME, 1, &Value::Blob(data.txn_id), "txn_id");
    bind_with_log(
        &stmt,
        FUNCTION_NAME,
        2,
    ...
    // FIXME - Properly handle conversion
    bind_with_log(
        &stmt,
        FUNCTION_NAME,
        3,
        &Value::Int64(data.slot as i64),
        "slot",
    );
    ...
    bind_with_log(
        ...
        FUNCTION_NAME,
        4,
        &Value::Int32(data.txn_idx as i32),
        "txn_idx",
    );
    ...
    bind_with_log(
        ...
        &data
            .prv_txn_id
            .as_ref()
            .map(|prv_txn_id| Value::Blob(prv_txn_id.clone()))
            .unwrap_or(Value::Null),
        "prv_txn_id",
    );
