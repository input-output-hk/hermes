...
// This is redundant and should be move out to shared when doing rbac refactor.
...
use crate::rbac::get_rbac::get_rbac_chain_from_cat_id;

/// Captures just the digits after last slash.
/// Use to filter out the rotation and role in catalyst ID.
...
impl CatalystRBACTokenV1 {
    ...
    pub(crate) fn new(
        ...
        // Create a catid and set nonce to the current time
    ...
    pub(crate) fn parse(token: &str) -> anyhow::Result<CatalystRBACTokenV1> {
        let token = token
            ...
            .ok_or_else(|| anyhow::anyhow!("Missing token prefix"))?;
        let (token, signature) = token
            .rsplit_once('.')
            .ok_or_else(|| anyhow::anyhow!("Missing token signature"))?;
        let signature = BASE64_URL_SAFE_NO_PAD
            .decode(signature.as_bytes())
            .context("Invalid token signature encoding")?
            .try_into()
            .map(|b| Signature::from_bytes(&b))
            .map_err(|_| anyhow::anyhow!("Invalid token signature length"))?;
        ...
        if catalyst_id.username().is_some_and(|n| !n.is_empty()) {
            return Err(anyhow::anyhow!("Catalyst ID must not contain username"));
        }
        if !catalyst_id.clone().is_id() {
            return Err(anyhow::anyhow!("Catalyst ID must be in an ID format"));
        }
        if catalyst_id.nonce().is_none() {
            return Err(anyhow::anyhow!("Catalyst ID must have nonce"));
        }
    ...
    pub(crate) fn reg_chain(
        ...
        if self.reg_chain.is_none() {
            let Some((reg_chain, _)) = get_rbac_chain_from_cat_id(
                persistent,
                volatile,
...
fn convert_network((network, subnet): &(String, Option<String>)) -> anyhow::Result<Network> {
    if network != "cardano" {
        return Err(anyhow::anyhow!("Unsupported network: {network}"));
    }
    ...
    match subnet.as_deref() {
        ...
        Some(subnet) => Err(anyhow::anyhow!("Unsupported host: {subnet}.{network}",)),
    }
}
...
mod tests {
    ...
    fn is_young() {
        ...
        // Move timestamp 2s to the past (chrono duration)
        token.catalyst_id = token
            .catalyst_id
            .with_specific_nonce(now - ChronoDuration::seconds(2));

        // std::time::Duration for is_young()
        ...
        // Move timestamp 2s to the future
        token.catalyst_id = token
            .catalyst_id
            .with_specific_nonce(now + ChronoDuration::seconds(2));
